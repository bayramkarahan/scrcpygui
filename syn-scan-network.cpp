...
void start_scan()
{
    char* nip="192.168.1.0/16";
    char* nport="5555";
    NetworkInterface iface = NetworkInterface::default_interface();
    std::cout << "Running on interface: " << iface.name() << "\n";
    std::cout << "SYN scan [" << nip << "]:[" << nport << "]\n";

    AddressRange<IPv4Address> target_addresses = parse_target(nip);

    char* port_list = (char*)malloc(strlen(nport) + 1);
    strcpy(port_list, nport);
